      ...
      >
        <View style={styles.card}>
          <View style={styles.imageWrap}>
            {!loaded && <View style={[styles.skeleton, { width: imageWidth, height: imageWidth }]} />}
            <Image
...
export default function GalleryScreen() {
  ...
  const onMomentumEnd = useCallback(({ nativeEvent: { layoutMeasurement, contentOffset, contentSize } }) => {
    if (layoutMeasurement.height + contentOffset.y >= contentSize.height - 400) handleLoadMore();
  }, [handleLoadMore]);
  ...
  return (
    <FlatList
      ...
      renderItem={({ item, index }) => (
        <ArtworkCard item={item} onPress={handlePress} imageWidth={imageWidth} index={index} />
      )}
      ...
      ListFooterComponent={hasMore ? <View style={styles.footer} /> : <View style={styles.footerEnd} />}
...
const styles = StyleSheet.create({
  ...
  skeleton: { backgroundColor: '#E8E5E2', position: 'absolute' },
